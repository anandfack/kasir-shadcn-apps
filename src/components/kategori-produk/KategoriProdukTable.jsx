...
import KategoriProdukDialog from "./KategoriProdukDialog";
import KategoriProdukActions from "./KategoriProdukActions";
import useFetchKategoriProduk from "@/hooks/kategori-produk/useFetchKategoriProduk";
...
const KategoriProdukTable = () => {
  ...
  const { data, loading, error } = useFetchKategoriProduk(
    "/api/v1/admin/kategori-produk",
    refreshKey
  );
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      await apiRequest(
        "DELETE",
        `/api/v1/admin/kategori-produk/${deleteData.id}`
      );
      toast({
        title: "Sukses!",
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="capitalize">{row.getValue("kode_kategori")}</div>
        ),
      },
      {
