...
const AdminPurchaseOrderTable = () => {
  ...
  const { data, loading, error } = useFetchAdminPurchaseOrder(
    "/api/v1/admin/purchase-order",
    refreshKey,
  );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <div className="capitalize">{row.getValue("nomor_po")}</div>
        ),
      },
      {
        id: "supplier",
        ...
        cell: ({ row }) => {
          return (
            <div className="capitalize">
              {formatTanggalTanpaJam(row.getValue("tanggal_po"))}
            </div>
          );
        },
      },
      {
        accessorKey: "status_po",
        ...
        cell: ({ row }) => {
          const status = row.getValue("status_po");
...
export default AdminPurchaseOrderTable;
